...
execute_iinstr!(
    addi,
    AddiIInstr,
    |rs1_val, sext_imm, shamt| (rs1_val as i32).wrapping_add(sext_imm) as u32
);
...
execute_iinstr!(slti, SltiIInstr, |rs1_val, sext_imm, shamt| {
    if (rs1_val as i32) < sext_imm { 1 } else { 0 }
});
...
execute_iinstr!(slli, SlliIInstr, |rs1_val, sext_imm, shamt| rs1_val
    << shamt);
...
execute_iinstr!(srli, SrliIInstr, |rs1_val, sext_imm, shamt| rs1_val
    >> shamt);
...
execute_iinstr!(
    srai,
    SraiIInstr,
    |rs1_val, sext_imm, shamt| ((rs1_val as i32) >> shamt) as u32
);
...
impl Execute for InstrI {
    fn execute(&self, steps: &mut [InstrStep; 8]) -> usize {
        ...
        match raw_masked {
            x if x == (AddiIInstr::expected_pattern() & I_TYPE_PATTERN_MASK) => {
                AddiIInstr::new(raw).execute(steps)
            }
            x if x == (SltiIInstr::expected_pattern() & I_TYPE_PATTERN_MASK) => {
                SltiIInstr::new(raw).execute(steps)
            }
            x if x == (SltuiIInstr::expected_pattern() & I_TYPE_PATTERN_MASK) => {
                SltuiIInstr::new(raw).execute(steps)
            }
            x if x == (XoriIInstr::expected_pattern() & I_TYPE_PATTERN_MASK) => {
                XoriIInstr::new(raw).execute(steps)
            }
            x if x == (OriIInstr::expected_pattern() & I_TYPE_PATTERN_MASK) => {
                OriIInstr::new(raw).execute(steps)
            }
            x if x == (AndiIInstr::expected_pattern() & I_TYPE_PATTERN_MASK) => {
                AndiIInstr::new(raw).execute(steps)
            }
            x if x == (SlliIInstr::expected_pattern() & I_TYPE_PATTERN_MASK)
                && funct7 == ITINSTR_SLLI_FUNCT7 =>
            {
                SlliIInstr::new(raw).execute(steps)
            }
            x if x == (SrliIInstr::expected_pattern() & I_TYPE_PATTERN_MASK)
                && funct7 == ITINSTR_SRLI_FUNCT7 =>
            {
                SrliIInstr::new(raw).execute(steps)
            }
            x if x == (SraiIInstr::expected_pattern() & I_TYPE_PATTERN_MASK)
                && funct7 == ITINSTR_SRAI_FUNCT7 =>
            {
                SraiIInstr::new(raw).execute(steps)
            }
            _ => {
